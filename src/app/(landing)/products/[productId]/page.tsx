...
import { fetchSinglePro } from '@/app/data/dynamicData'
import { colors, productData } from '@/app/data/staticData'
import Image from 'next/image'
import { BusFront, CatIcon, Check, DollarSign, Heart, IndianRupee, Minus, Plus, Settings, ShoppingBagIcon, Star, StarHalf } from 'lucide-react'
                    ...
                    <div className="flex flex-col md:flex-row gap-2 lg:gap-5 w-full h-full">
                        ...
                        <div className='bg-bg-gray p-5 rounded-lg w-full lg:h-full md:w-full lg:max-w-[700px] flex flex-col gap-4'>
                            <div className='flex w-full flex-col gap-2 mt-2 '>
                                <div>
                                    <h1 className='text-20 md:text-24  uppercase text-text2 font-semibold'>PROduct NAmE</h1>
                                    <h3 className='text-18 md:text-20  uppercase text-text2 flex items-center gap-2 '><IndianRupee className='h-4 w-4' /> 125</h3>
                                </div>
                            ...
                            <div className='qauntity w-full flex flex-col gap-2'>
                                <h4 className='text-14 md:text-18 text-black font-semibold '>Quantity</h4>
                                <div className='Quantity border border-secoundary flex text-secoundary h-8 w-36 font-semibold '>
                                    <button className='h-full w-full flex text-14 md:text-16 justify-center items-center bg-white hover:text-black hover:bg-black/20' onClick={() => setquantity((quantity) => quantity === 1 ? quantity : quantity - 1)}>
                                        <Minus />
                                    </button>
                                    <div className='h-full w-full flex text-14 md:text-16 justify-center items-center'>{quantity}</div>
                                    <button className='h-full w-full flex text-14 md:text-16 justify-center items-center bg-white hover:text-black hover:bg-black/20' onClick={() => setquantity(quantity + 1)}>
                                        <Plus />
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className='colors w-full flex flex-col gap-2'>
                                <h4 className='text-14 md:text-18 text-black font-semibold '>Colors</h4>
                            ...
                            <div className='features w-full flex flex-col gap-2'>
                                <h4 className='text-14 md:text-18 text-black font-semibold '>Features</h4>
                            ...
                            <div className="actions flex flex-col sm:flex-row md:flex-col lg:flex-row w-full gap-2 ">
                                <button className='w-full h-10 z-20 text-14 md:text-16 uppercase  text-white relative bg-brown hover:bg-transparent before:absolute before:-z-10 before:top-0 before:start-0 before:w-full before:h-0 before:hover:bg-black before:hover:w-full before:hover:h-full before:transition-all before:duration-300 before:ease-linear rounded-none transtition-all duration-300 ease-linear'>Add To Cart</button>
