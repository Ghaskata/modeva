...
const Header = (props: Props) => {
    ...
    return (
        <div className='header w-screen h-[120px] mx-auto fixed bg-transparent transition-all duration-500 ease-linear z-50'>
            {
                openCatalog && <div className='backdrop absolute top-0 start-0 h-screen w-screen bg-black/30' onClick={() => setopenCatalog(false)}></div>
            }
            <div className='container mx-auto flex justify-between items-center h-full px-5 sm:px-0' >
                <Link href={"/"} className="logo group/logo ">
                    <h1 className='uppercase text-42 font-semibold text-text1 group-hover/logo:text-brown transition-all duration-500 ease-linear '><span className='text-brown group-hover/logo:text-text1 transition-all duration-500 ease-linear '>M</span>odeva</h1>
                </Link>
                <div className="navbar__part w-full hidden xl:block">
                    <ul className='relative flex justify-between items-center w-1/2 mx-auto text-14 font-popins' >
                        <button className='flex items-center justify-center gap-4 text-text1 hover:text-brown cursor-pointer transition-all duration-300 ease-linear' onClick={() => setopenCatalog(true)}>
                            <h3 className=''>Catalog</h3>
                            {
                                !openCatalog ? <ChevronDown /> : <ChevronUp />
                            }
                        </button>
                        <CatalogCompo openCatalog={openCatalog} setopenCatalog={setopenCatalog} />
                ...
                <div className="relative actions__part">
                    <div className="flex gap-2 md:gap-4 items-center justify-end">
                        <div className="relative searchPart hidden lg:block">
                            ...
                            <input type="text" placeholder='search' className='ps-10 py-1 rounded w-[250px] border-0 focus:border-0 focus:outline-none' />
                        </div>
                        <button className='text-white block lg:hidden'>
                            <Search className='h-6 w-6' />
                        </button>
                        <button className='text-white group/profile' onClick={() => setopenProfile(!openProfile)}>
                        ...
                        <button className='text-white  group/cart' onClick={() => setopenCart(!openCart)}>
